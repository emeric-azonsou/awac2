...
export async function submitVote(deps: VoteDeps, body: unknown): Promise<HttpResult> {
  ...
  // La devise est fixée côté serveur (réglages), jamais depuis la requête client :
  // le montant est calculé en XOF, une devise cliente incohérente est un risque métier.
  ...
  // On persiste la ligne de vote (pending) AVANT de déclencher le paiement : ainsi
  // toute collecte acceptée possède une ligne réconciliable (aucun paiement orphelin
  // si le processus s'interrompt après l'appel SebPay).
...
// Réconcilie un vote pending avec le statut réel de la collecte SebPay (source de
// vérité authentifiée). Partagé par le polling, la page reçu et le cron : la
// confirmation reste idempotente via confirmVote. Les erreurs SebPay remontent
// à l'appelant, qui décide (garder pending, réessayer plus tard).
