...
function makeDb({ candidateExists = true, voteCount = 10, voteStatus = 'pending' } = {}) {
  ...
  const run = (strings, params) => {
    ...
    if (sql.includes('INSERT INTO votes')) {
      state.vote.receipt_code = params.find((p) => typeof p === 'string' && p.startsWith('AWAC-'))
    ...
    if (sql.includes('UPDATE candidates')) { state.voteCount = params[0]; state.vote.votes_after = params[0]; return [] }
    if (sql.includes('UPDATE votes')) {
      const terminal = params.find((p) => p === 'confirmed' || p === 'rejected')
      if (terminal) state.vote.payment_status = terminal
      return [{ ...state.vote }]
    }
    return []
  }
  const db = (strings, ...params) => Promise.resolve(run(strings, params))
  db.begin = async (fn) => fn((strings, ...params) => Promise.resolve(run(strings, params)))
  db._state = state
  return db
}

function post(app, body, path = '/votes') {
...
describe('POST /votes — mode réel (SebPay configuré)', () => {
  it('crée un vote pending et appelle SebPay, renvoie provider_link', async () => {
    const db = makeDb()
    const sebpay = {
      createCollection: vi.fn().mockResolvedValue({ transaction_id: 'sp_1', status: 'pending', provider_link: 'https://pay.sebpay/x' }),
    }
    const app = buildTestApp({ db, sebpay }, (a) => a.route('/votes', votesRouter))
    ...
    const [args] = sebpay.createCollection.mock.calls[0]
  ...
  it('renvoie 502 si SebPay refuse (numéro invalide)', async () => {
    const db = makeDb()
    const sebpay = { createCollection: vi.fn().mockRejectedValue(new Error('SebPay collections a échoué : Numéro invalide')) }
    const app = buildTestApp({ db, sebpay }, (a) => a.route('/votes', votesRouter))
    const res = await post(app, validBody)
  ...
  it('rejette les entrées invalides avant tout appel SebPay', async () => {
    const sebpay = { createCollection: vi.fn() }
    const app = buildTestApp({ db: makeDb(), sebpay }, (a) => a.route('/votes', votesRouter))
    for (const body of [
    ...
    expect(sebpay.createCollection).not.toHaveBeenCalled()
  })
})
...
describe('GET /votes/:id/status — polling', () => {
  it('réconcilie via SebPay et confirme quand approved', async () => {
    ...
    const sebpay = { getCollection: vi.fn().mockResolvedValue({ transaction_id: 'sp_1', status: 'approved' }) }
    const app = buildTestApp({ db, sebpay }, (a) => a.route('/votes', votesRouter))
    const res = await app.request(`/votes/${VOTE_ID}/status`)
    expect(res.status).toBe(200)
    const body = await res.json()
    expect(body.payment_status).toBe('confirmed')
    expect(sebpay.getCollection).toHaveBeenCalledWith('AWAC-known')
  })

  it('ne rappelle pas SebPay si le vote est déjà confirmé', async () => {
    const db = makeDb({ voteStatus: 'confirmed' })
    const sebpay = { getCollection: vi.fn() }
    const app = buildTestApp({ db, sebpay }, (a) => a.route('/votes', votesRouter))
    const res = await app.request(`/votes/${VOTE_ID}/status`)
    expect((await res.json()).payment_status).toBe('confirmed')
    expect(sebpay.getCollection).not.toHaveBeenCalled()
  })

  it('reste pending si SebPay est injoignable (erreur transitoire)', async () => {
    const db = makeDb({ voteStatus: 'pending' })
    const sebpay = { getCollection: vi.fn().mockRejectedValue(new Error('timeout')) }
    const app = buildTestApp({ db, sebpay }, (a) => a.route('/votes', votesRouter))
    const res = await app.request(`/votes/${VOTE_ID}/status`)
    expect(res.status).toBe(200)
    expect((await res.json()).payment_status).toBe('pending')
  })
})
